...
FIXTURE = json.loads(
    (Path(__file__).parent.parent / "fixtures" / "hackernews_hits.json").read_text()
)
...
def _make_collector(lookback: int = 999_999_999):
    ...
    return HackerNewsCollector(lookback_seconds=lookback)
...
async def test_collect_maps_fields_correctly():
    ...
    assert first.external_id == "39500001"
    assert "ycombinator" in first.url or "example.com" in first.url
    assert first.title != ""
...
@pytest.mark.asyncio
async def test_deduplication_across_queries():
    """Same objectID returned by multiple queries should appear only once."""
    collector = _make_collector()
    # Return the same hit for every query call
    with patch("httpx.AsyncClient") as mock_client_class:
        mock_client = AsyncMock()
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        mock_client.__aexit__ = AsyncMock(return_value=False)
    ...
    # 3 hits in fixture, multiple queries, but IDs should be deduplicated
